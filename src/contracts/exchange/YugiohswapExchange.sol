...
import "../interfaces/IYugiohswapExchange.sol";
 ...
 */
contract YugiohswapExchange is ReentrancyGuard, ERC1155MintBurn, IYugiohswapExchange {
  ...
  constructor(address _tokenAddr, address _currencyAddr, uint256 _currencyID) public {
    require(
      ...
      "YugiohswapExchange#constructor:INVALID_INPUT"
    );
    factory = msg.sender;
  ...
  {
    // Input validation
    require(_deadline >= block.timestamp, "YugiohswapExchange#_currencyToToken: DEADLINE_EXCEEDED");
    ...
    for (uint256 i = 0; i < nTokens; i++) {
      ...
      require(amountBought > 0, "YugiohswapExchange#_currencyToToken: NULL_TOKENS_BOUGHT");
   ...
   * @return price Amount of currency tokens to send to Yugiohswap.
   */
    ...
    override public pure returns (uint256 price)
  {
    ...
    require(_assetSoldReserve > 0 && _assetBoughtReserve > 0, "YugiohswapExchange#getBuyPrice: EMPTY_RESERVE");
    ...
    internal nonReentrant() returns (uint256[] memory currencyBought)
  {
    ...
    // Input validation
    require(_deadline >= block.timestamp, "YugiohswapExchange#_tokenToCurrency: DEADLINE_EXCEEDED");
    ...
    for (uint256 i = 0; i < nTokens; i++) {
      ...
      require(amountSold > 0, "YugiohswapExchange#_tokenToCurrency: NULL_TOKENS_SOLD");
    ...
    require(totalCurrency >= _minCurrency, "YugiohswapExchange#_tokenToCurrency: INSUFFICIENT_CURRENCY_AMOUNT");
   ...
   * @return price Amount of currency tokens to receive from Yugiohswap.
   */
  ...
  {
    //Reserves must not be empty
    require(_assetSoldReserve > 0 && _assetBoughtReserve > 0, "YugiohswapExchange#getSellPrice: EMPTY_RESERVE");
    ...
    return numerator / denominator; //Rounding errors will favor Yugiohswap, so nothing to do
  }
  ...
  {
    // Requirements
    require(_deadline >= block.timestamp, "YugiohswapExchange#_addLiquidity: DEADLINE_EXCEEDED");
    ...
    for (uint256 i = 0; i < nTokens; i ++) {
      ...
      require(_maxCurrency[i] > 0, "YugiohswapExchange#_addLiquidity: NULL_MAX_CURRENCY");
      require(amount > 0, "YugiohswapExchange#_addLiquidity: NULL_TOKENS_AMOUNT");

      // If the token contract and currency contract are the same, prevent the creation
      // of a currency pool.
      if (currencyPoolBanned) {
        require(tokenId != currencyID, "YugiohswapExchange#_addLiquidity: CURRENCY_POOL_FORBIDDEN");
      }
      ...
      if (totalLiquidity > 0) {
        ...
        */
        ...
        require(_maxCurrency[i] >= currencyAmount, "YugiohswapExchange#_addLiquidity: MAX_CURRENCY_AMOUNT_EXCEEDED");
        ...
        // See https://github.com/0xsequence/yugiohswap/issues/19
        ...
        require(maxCurrency >= 1000000000, "YugiohswapExchange#_addLiquidity: INVALID_CURRENCY_AMOUNT");
  ...
  {
    // Input validation
    require(_deadline > block.timestamp, "YugiohswapExchange#_removeLiquidity: DEADLINE_EXCEEDED");
    ...
    for (uint256 i = 0; i < nTokens; i++) {
      ...
      require(totalLiquidity > 0, "YugiohswapExchange#_removeLiquidity: NULL_TOTAL_LIQUIDITY");
      ...
      require(currencyAmount >= _minCurrency[i], "YugiohswapExchange#_removeLiquidity: INSUFFICIENT_CURRENCY_AMOUNT");
      require(tokenAmount >= _minTokens[i], "YugiohswapExchange#_removeLiquidity: INSUFFICIENT_TOKENS");
    ...
    override public returns(bytes4)
  {
    ...
    // Users must be responsible and only use this Yugiohswap exchange
    ...
    |__________________________________*/
      ...
      // Tokens received need to be YUGIOH-1155 tokens
    ...
    |__________________________________*/
      ...
      require(msg.sender == address(currency), "YugiohswapExchange#onERC1155BatchReceived: INVALID_TOKENS_DEPOSITED");
      require(_ids[0] == currencyID, "YugiohswapExchange#onERC1155BatchReceived: INVALID_CURRENCY_ID");

    } else {
      revert("YugiohswapExchange#onERC1155BatchReceived: INVALID_METHOD");
    }
    ...
    override public returns(bytes4)
  {
    ...
    require(
      ...
      "YugiohswapExchange#onERC1155Received: INVALID_ONRECEIVED_MESSAGE"
    );
   ...
   */
  fallback () external {
    revert("YugiohswapExchange:UNSUPPORTED_METHOD");
  }
    ...
    internal view returns (uint256[] memory)
  {
    ...
    if (nTokens == 1) {
      ...
      for (uint256 i = 1; i < nTokens; i++) {
        require(_tokenIds[i-1] < _tokenIds[i], "YugiohswapExchange#_getTokenReserves: UNSORTED_OR_DUPLICATE_TOKEN_IDS");
