...
/**
 * @notice Will allow users to wrap their  ERC-20 into BEP-1155 tokens
 *         and pass their order to yugiohswap. All funds will be returned
...
contract WrapAndNiftyswap {
  ...
  IERC20Wrapper immutable public tokenWrapper; // ERC-20 to BEP-1155 token wrapper contract
  address immutable public exchange;           // Niftyswap exchange to use
  address immutable public erc20;              // ERC-20 used in yugiohswap exchange
  address immutable public erc1155;            // BEP-1155 used in yugiohswap exchange

  uint256 immutable internal wrappedTokenID; // ID of the wrapped token
  bool internal isInNiftyswap;               // Whether yugiohswap is being called
   ...
   * @param _yugiohswapOrder  Encoded Niftyswap order passed in data field of safeTransferFrom()
   */
  function wrapAndSwap(
    uint256 _maxAmount,
    address _recipient,
    bytes calldata _yugiohswapOrder
  ) external
  {
    // Decode yugiohswap order
    INiftyswapExchange.BuyTokensObj memory obj;
    (, obj) = abi.decode(_yugiohswapOrder, (bytes4, INiftyswapExchange.BuyTokensObj));
    ...
    // Swap on Niftyswap
    isInNiftyswap = true;
    tokenWrapper.safeTransferFrom(address(this), exchange, wrappedTokenID, _maxAmount, _yugiohswapOrder);
    isInNiftyswap = false;
  ...
  function onERC1155BatchReceived(
    ...
    external returns(bytes4)
  { 
    // If coming from yugiohswap or wrapped token, ignore
    ...
    (,obj) = abi.decode(_yugiohswapOrder, (bytes4, INiftyswapExchange.SellTokensObj));
    ...
    // Swap on Niftyswap
    isInNiftyswap = true;
    IERC1155(msg.sender).safeBatchTransferFrom(address(this), exchange, _ids, _amounts, _yugiohswapOrder);
    isInNiftyswap = false;
