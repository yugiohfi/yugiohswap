...
describe('YugiohswapExchange', () => {
  ...
  let operatorExchangeContract: YugiohswapExchange
  ...
  conditions.forEach(function(condition) {
    context(condition[0] as string, () => {
      ...
      beforeEach(async () => {
        ...
        await yugiohswapFactoryContract.functions.createExchange(
        ...
        yugiohswapExchangeContract = new ethers.Contract(exchangeAddress, exchangeABI, ownerWallet) as YugiohswapExchange
        operatorExchangeContract = yugiohswapExchangeContract.connect(operatorSigner) as YugiohswapExchange
        ...
        await operatorCurrencyContract.functions.setApprovalForAll(yugiohswapExchangeContract.address, true)
        await operatorERC1155Contract.functions.setApprovalForAll(yugiohswapExchangeContract.address, true)
        await userCurrencyContract.functions.setApprovalForAll(yugiohswapExchangeContract.address, true)
        await userERC1155Contract.functions.setApprovalForAll(yugiohswapExchangeContract.address, true)
      })

      describe('Getter functions', () => {
        describe('getTokenAddress() function', () => {
          it('should return token address', async () => {
            const token_address = await yugiohswapExchangeContract.functions.getTokenAddress()
        ...
        describe('getCurrencyInfo() function', () => {
          it('should return currency token address and ID', async () => {
            const token_info = await yugiohswapExchangeContract.functions.getCurrencyInfo()
        ...
        describe('getBuyPrice() function', () => {
          it('should round UP', async () => {
            ...
            let numerator = 1500
            let denominator = 751
            const price = await yugiohswapExchangeContract.functions.getBuyPrice(bought_amount, numerator, denominator)
        ...
        describe('getSellPrice() function', () => {
          it('should round DOWN', async () => {
            ...
            let denominator = 751
            let bought_amount = 100
            const price = await yugiohswapExchangeContract.functions.getSellPrice(bought_amount, denominator, numerator)
        ...
        describe('getFactoryAddress() function', () => {
          it('should return factory address', async () => {
            const factory_address = await yugiohswapExchangeContract.functions.getFactoryAddress()
            await expect(factory_address[0]).to.be.eql(yugiohswapFactoryContract.address)
          })
        })

        describe('supportsInterface()', () => {
          it('should return true for 0x01ffc9a7 (IERC165)', async () => {
            const support = await yugiohswapExchangeContract.functions.supportsInterface('0x01ffc9a7')
            expect(support[0]).to.be.eql(true)
          })

          it('should return true for 0x4e2312e0 (IERC1155Receiver)', async () => {
            const support = await yugiohswapExchangeContract.functions.supportsInterface('0x4e2312e0')
            expect(support[0]).to.be.eql(true)
          })

          it('should return true for 0xd9b67a26 (IERC1155)', async () => {
            const support = await yugiohswapExchangeContract.functions.supportsInterface('0xd9b67a26')
            expect(support[0]).to.be.eql(true)
          })
        })
      })
      ...
      describe('_addLiquidity() function', () => {
        it('should pass when balances are sufficient', async () => {
          const tx = operatorERC1155Contract.functions.safeBatchTransferFrom(
            operatorAddress,
            yugiohswapExchangeContract.address,
            types,
            tokenAmountsToAdd,
            addLiquidityData,
            { gasLimit: 50000000 }
        ...
        it('should ROUND UP the currency amount to be deposited on second deposit', async () => {
          ...
          let reserve1 = (await yugiohswapExchangeContract.functions.getCurrencyReserves([0]))[0]
          ...
          let reserve2 = (await yugiohswapExchangeContract.functions.getCurrencyReserves([0]))[0]
        ...
        it('should ROUND DOWN the amount of liquidity to mint on second deposit', async () => {
          ...
          let liquidity_supply1 = (await yugiohswapExchangeContract.functions.getTotalSupply([0]))[0]
          ...
          let liquidity_supply2 = (await yugiohswapExchangeContract.functions.getTotalSupply([0]))[0]
        ...
        it('should REVERT if trying to crease currency/currency pool', async () => {
          ...
          // Retrieve exchange address
          let exchangeAddress2 = await yugiohswapFactoryContract.functions.tokensToExchange(
            ownerERC1155Contract.address,
            ownerERC1155Contract.address,
            types[0]
          )
          ...
          await expect(tx).to.be.rejectedWith(RevertError('YugiohswapExchange#_addLiquidity: CURRENCY_POOL_FORBIDDEN'))
        })
        ...
        it('should REVERT if deadline is passed', async () => {
          ...
          await expect(tx).to.be.rejectedWith(RevertError('YugiohswapExchange#_addLiquidity: DEADLINE_EXCEEDED'))
        })
        ...
        it('should REVERT if a maxCurrency is null', async () => {
          ...
          await expect(tx).to.be.rejectedWith(RevertError('YugiohswapExchange#_addLiquidity: NULL_MAX_CURRENCY'))
        })
        ...
        it('should REVERT if a token amount is null', async () => {
          ...
          await expect(tx).to.be.rejectedWith(RevertError('YugiohswapExchange#_addLiquidity: NULL_TOKENS_AMOUNT'))
        })
        ...
        it('should REVERT if any duplicate', async () => {
          ...
          await expect(tx1).to.be.rejectedWith(
            RevertError('YugiohswapExchange#_getTokenReserves: UNSORTED_OR_DUPLICATE_TOKEN_IDS')
          )
        ...
        context('When liquidity was added', () => {
          ...
          beforeEach(async () => {
            tx = await operatorERC1155Contract.functions.safeBatchTransferFrom(
              operatorAddress,
              yugiohswapExchangeContract.address,
              types,
              tokenAmountsToAdd,
          ...
          it('should update currency balances', async () => {
            const exchangeBalance = await userCurrencyContract.functions.balanceOf(yugiohswapExchangeContract.address, currencyID)
          ...
          it('should update the currency per token reserve', async () => {
            for (let i = 0; i < types.length; i++) {
              const reserve = await yugiohswapExchangeContract.functions.getCurrencyReserves([types[i]])
              expect(reserve[0][0]).to.be.eql(currencyAmountToAdd)
            }
          })

          it('should update YugiohSwap Token ids balances', async () => {
            for (let i = 0; i < types.length; i++) {
              const exchangeBalance = await yugiohswapExchangeContract.functions.balanceOf(
                yugiohswapExchangeContract.address,
                types[i]
              )
              const operatorBalance = await yugiohswapExchangeContract.functions.balanceOf(operatorAddress, types[i])
          ...
          it('should update total supplies for Yugiohswap Token ids balances', async () => {
            const exchangeTotalSupplies = await yugiohswapExchangeContract.functions.getTotalSupply(types)
            for (let i = 0; i < types.length; i++) {
          ...
          it('should DECREASE the BUY prices for 2ND deposit', async () => {
            ...
            // After 1st deposit
            const prePrices = await yugiohswapExchangeContract.functions.getPrice_currencyToToken(types, ones)
            ...
            const postPrices = await yugiohswapExchangeContract.functions.getPrice_currencyToToken(types, ones)
          ...
          it('should INCREASE the SELL prices for 2ND deposit', async () => {
            ...
            // After 1st deposit
            const prePrices = await yugiohswapExchangeContract.functions.getPrice_tokenToCurrency(types, ones)
            ...
            const postPrices = await yugiohswapExchangeContract.functions.getPrice_tokenToCurrency(types, ones)
          ...
          it('should emit LiquidityAdded event', async () => {
            ...
            filterFromOperatorContract = yugiohswapExchangeContract.filters.LiquidityAdded(null, null, null, null)
            ...
            expect(logs[0].topics[0]).to.be.eql(
              yugiohswapExchangeContract.interface.getEventTopic(
                yugiohswapExchangeContract.interface.events['LiquidityAdded(address,uint256[],uint256[],uint256[])']
              )
            )
          })
        })
        ...
        context('When liquidity was added for the second time', () => {
          ...
          it('should REVERT if a maxCurrency is exceeded', async () => {
            ...
            await expect(tx).to.be.rejectedWith(RevertError('YugiohswapExchange#_addLiquidity: MAX_CURRENCY_AMOUNT_EXCEEDED'))
          })
          ...
          it('should update total supples for Yugiohswap Token ids balances', async () => {
            for (let i = 0; i < types.length; i++) {
              const exchangeTotalSupply = await yugiohswapExchangeContract.functions.getTotalSupply([types[i]])
              const newCurrencyAmount = tokenAmountToAdd.mul(currencyAmountToAdd).div(tokenAmountToAdd)
      ...
      describe('_removeLiquidity() function', () => {
        ...
        const yugiohTokenToSend = new Array(nTokenTypesToRemove).fill('').map((a, i) => currencyAmountToRemove)

        const removeLiquidityData: string = getRemoveLiquidityData(currencyAmountsToRemove, tokenAmountsToRemove, deadline)

        it('should revert if no Yugiohswap token', async () => {
        ...
        it('should revert if empty reserve', async () => {
          ...
          await expect(tx).to.be.rejectedWith(RevertError('YugiohswapExchange#_removeLiquidity: NULL_TOTAL_LIQUIDITY'))
        })
        ...
        context('When liquidity was added', () => {
          ...
          it('should revert if insufficient currency', async () => {
            ...
            await expect(tx).to.be.rejectedWith(RevertError('YugiohswapExchange#_removeLiquidity: INSUFFICIENT_CURRENCY_AMOUNT'))
          })
          ...
          it('should revert if insufficient tokens', async () => {
            ...
            await expect(tx).to.be.rejectedWith(RevertError('YugiohswapExchange#_removeLiquidity: INSUFFICIENT_TOKENS'))
          })
          ...
          it('should PASS if enough Yugiohswap token', async () => {
            const tx = operatorExchangeContract.functions.safeBatchTransferFrom(
              operatorAddress,
              yugiohswapExchangeContract.address,
              typesToRemove,
          ...
          context('When liquidity was removed', () => {
            ...
            it('should update the currency amount per token reserve', async () => {
              ...
              const reserves = await yugiohswapExchangeContract.functions.getCurrencyReserves(types)
              for (let i = 0; i < types.length; i++) {
      ...
      describe('_tokenToCurrency() function', () => {
        ...
        it('should fail if token sent is 0', async () => {
          ...
          await expect(tx).to.be.rejectedWith(RevertError('YugiohswapExchange#_tokenToCurrency: NULL_TOKENS_SOLD'))
        })
        ...
        it('should fail if deadline is passed', async () => {
          ...
          await expect(tx).to.be.rejectedWith(RevertError('YugiohswapExchange#_tokenToCurrency: DEADLINE_EXCEEDED'))
        })
        ...
        it('should fail if currency balance is lower than cost', async () => {
          ...
          await expect(tx).to.be.rejectedWith(RevertError('YugiohswapExchange#_tokenToCurrency: INSUFFICIENT_CURRENCY_AMOUNT'))
        })
        ...
        describe('When trade is successful', async () => {
          ...
          it('should emit CurrencyPurchase event', async () => {
            ...
            // Get event filter to get internal tx event
            filterFromOperatorContract = yugiohswapExchangeContract.filters.CurrencyPurchase(null, null, null, null, null)
            ...
            expect(logs[0].topics[0]).to.be.eql(
              yugiohswapExchangeContract.interface.getEventTopic(
                yugiohswapExchangeContract.interface.events['CurrencyPurchase(address,address,uint256[],uint256[],uint256[])']
              )
            )
          })
        })
      ...
      describe('_currencyToToken() function', () => {
        ...
        it('should fail if a bought amount is 0', async () => {
          ...
          await expect(tx).to.be.rejectedWith(RevertError('YugiohswapExchange#_currencyToToken: NULL_TOKENS_BOUGHT'))
        })
        ...
        it('should fail if deadline is passed', async () => {
          ...
          await expect(tx).to.be.rejectedWith(RevertError('YugiohswapExchange#_currencyToToken: DEADLINE_EXCEEDED'))
        })
        ...
        it('should fail if any duplicate', async () => {
          ...
          const tx1 = userCurrencyContract.functions.safeTransferFrom(
            userAddress,
            yugiohswapExchangeContract.address,
            currencyID,
            cost,
            invalid_buyTokenData1,
            { gasLimit: 8000000 }
          )
          ...
          const tx3 = userCurrencyContract.functions.safeTransferFrom(
            userAddress,
            yugiohswapExchangeContract.address,
            currencyID,
            cost,
            invalid_buyTokenData3,
            { gasLimit: 8000000 }
          )
          await expect(tx3).to.be.rejectedWith(
            RevertError('YugiohswapExchange#_getTokenReserves: UNSORTED_OR_DUPLICATE_TOKEN_IDS')
        ...
        describe('When trade is successful', async () => {
          ...
          beforeEach(async () => {
            tx = await userCurrencyContract.functions.safeTransferFrom(
              userAddress,
              yugiohswapExchangeContract.address,
              currencyID,
              cost,
          ...
          it('should have token sell price adjusted', async () => {
            const price = (await yugiohswapExchangeContract.functions.getPrice_currencyToToken([0], [tokenAmountToBuy]))[0]
          ...
          it('should emit TokensPurchase event', async () => {
            ...
            // Get event filter to get internal tx event
            filterFromOperatorContract = yugiohswapExchangeContract.filters.TokensPurchase(null, null, null, null, null)
            ...
            expect(logs[0].topics[0]).to.be.eql(
              yugiohswapExchangeContract.interface.getEventTopic(
                yugiohswapExchangeContract.interface.events['TokensPurchase(address,address,uint256[],uint256[],uint256[])']
              )
            )
          })
        })
      ...
      describe('Edge cases', () => {
        it('Pool can not go to zero token in reserve', async () => {
          ...
          let tx = yugiohswapExchangeContract.functions.getPrice_currencyToToken([types[0]], [1])
        ...
        it('Pool stuck at 1 token can go back up to normal with loss', async () => {
          ...
          let cost = (await yugiohswapExchangeContract.functions.getPrice_currencyToToken([types[0]], [amount_to_buy]))[0][0]
          ...
          let currency_reserve = (await yugiohswapExchangeContract.functions.getCurrencyReserves([types[0]]))[0]
          let token_reserve = await userERC1155Contract.functions.balanceOf(yugiohswapExchangeContract.address, types[0])
