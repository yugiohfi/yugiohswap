...
describe('YugiohswapFactory', () => {
  ...
  let yugiohswapExchangeAbstract: AbstractContract
  ...
  beforeEach(async () => {
    ...
    yugiohswapExchangeAbstract = await AbstractContract.fromArtifactName('YugiohswapExchange')
  ...
  describe('Getter functions', () => {
    describe('getExchange() function', () => {
      ...
      it('should return exchange address', async () => {
        const exchange_address = await yugiohswapFactoryContract.functions.tokensToExchange(
          ownerERC1155Contract.address,
          ownerBaseTokenContract.address,
          baseTokenID
        )
  ...
  describe('createExchange() function', () => {
    ...
    it('should REVERT if Token is 0x0', async () => {
      const tx = yugiohswapFactoryContract.functions.createExchange(ZERO_ADDRESS, ownerBaseTokenContract.address, baseTokenID, {
        gasLimit: 1000000
      })
      await expect(tx).to.be.rejectedWith(RevertError('YugiohswapExchange#constructor:INVALID_INPUT'))
    })

    it('should REVERT if Base Token is 0x0', async () => {
      const tx = yugiohswapFactoryContract.functions.createExchange(ownerERC1155Contract.address, ZERO_ADDRESS, baseTokenID, {
        gasLimit: 1000000
      })
      await expect(tx).to.be.rejectedWith(RevertError('YugiohswapExchange#constructor:INVALID_INPUT'))
    })
    ...
    it("should PASS if exchange doesn't exist yet", async () => {
      const tx = yugiohswapFactoryContract.functions.createExchange(
        ownerERC1155Contract.address,
        ownerBaseTokenContract.address,
        baseTokenID
      )
    ...
    it('should PASS if creating an exchange with a new base currency contract', async () => {
      const tx = yugiohswapFactoryContract.functions.createExchange(ownerERC1155Contract.address, userAddress, baseTokenID)
      await expect(tx).to.be.fulfilled
    })
    ...
    context('When successful transfer', () => {
      ...
      beforeEach(async () => {
        tx = await yugiohswapFactoryContract.functions.createExchange(
          ownerERC1155Contract.address,
          ownerBaseTokenContract.address,
          baseTokenID
        )
      ...
      it('should REVERT if creating an existing exchange', async () => {
        ...
        await expect(tx).to.be.rejectedWith(RevertError('YugiohswapFactory#createExchange: EXCHANGE_ALREADY_CREATED'))
      })
